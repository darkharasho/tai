...
//
//   ESC [ G / [0G / [1G   — cursor to column 1 (e.g. node REPL)
//   ESC [ <n>D where n>=2 — cursor back N columns (Python's readline; the
...
export function stripAnsi(str: string): string {
  return str.replace(CURSOR_REDRAW_RE, '\r').replace(ANSI_RE, '');
}
